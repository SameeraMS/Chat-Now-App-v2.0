...
public class ChatModel {
    ...
    public boolean saveChat(ChatDto dto) throws SQLException {
        ...
        return SqlUtil.execute(query,dto.getUsername(),dto.getId(),dto.getMessage(),dto.getImage());

    }

    public ArrayList<ChatDto> getChat(String username) throws SQLException {
        String query = "SELECT * FROM chat WHERE username = ?";
        ResultSet rs = SqlUtil.execute(query,username);
        ArrayList<ChatDto> list = new ArrayList<>();
        while (rs.next()){
            list.add(new ChatDto(rs.getString(1),rs.getString(2),rs.getString(3),rs.getBinaryStream(4)));
        }
        return list;
    }
}
